...
export default function SplashScreen() {
  ...
  // Characters for "CENTAVERSE" logo
  const logoText = "CENTAVERSE"

  useEffect(() => {
    const logo = logoRef.current
    if (!logo) return

    // Set initial state exactly as 93GRADOS does - translate(-50%, 100%) with opacity 0
    gsap.set(logo, {
    ...
    // Animate to center exactly as 93GRADOS does - translate(-50%, -50%) with opacity 1
    // Using the exact same animation approach as the original
    gsap.to(logo, {
      ...
      ease: "power3.out",
      onComplete: () => {
        // Set final state exactly as 93GRADOS does
        gsap.set(logo, {
          transform: "translate(-50%, -50%)",
          opacity: 1,
          visibility: "inherit",
